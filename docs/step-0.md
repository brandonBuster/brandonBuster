...
Step 0 establishes the monorepo and runnable frontend/backend with a basic contact form flow. It is directly tied to **roadmap.md** Phase 0 and **requirements.md** Sections 2 (Contact), 7 (Repository Structure), and 8 (Local Development).
...
| **requirements.md** Section 2 | Contact: form (name, email, message); API `POST /api/contact`; validation |
| **requirements.md** Section 7 | Monorepo: `/apps/web`, `/apps/api`, `/infra`, `/docs`, `/.github` |
| **requirements.md** Section 8 | Frontend: `npm run dev`; Backend: `uvicorn main:app --reload`; `.env.example` committed, `.env.local` gitignored |
...
- [x] **3.1** FastAPI app under `apps/api` with `POST /api/contact`  
  _→ roadmap Phase 0; requirements Sections 2, 4 (Backend)_
- [x] **3.2** Request body validated (name, email, message); appropriate status codes (e.g. 200, 422)  
  _→ requirements Section 4 (Input validation)_
...
- [x] **3.5** TODO or stub for email delivery (SendGrid); no secrets in repo  
  _→ requirements Sections 2, 3 (Security)_
...
- [x] **5.1** `infra/` has placeholder structure or README (e.g. `envs/dev`, `envs/prod`, `modules/` or note that Terraform is Phase 1)  
  _→ requirements Section 5 (Terraform structure); no full Terraform in Step 0_
- [x] **5.2** `.github/workflows/` exists or is documented as Phase 3  
  _→ requirements Sections 6, 7; CI/CD is Phase 3_
...
- [x] **6.2** Brief local dev instructions (how to run web + api) in README or `docs/`  
  _→ requirements Section 8 (Clear documentation for local dev)_
...
- **Rate limiting / spam:** Requirements Section 2 call for spam mitigation and rate limiting; these are better handled at edge/APIM (Phase 2). Step 0 only needs validation and a working request/response flow.
